...
systemfonts::register_font("Gilroy Medium",
                           plain = here("assets/fonts/Gilroy-Medium.otf"))
...
# Data ----
set.seed(444)
pmp_data <- readr::read_rds(here("data/pp_2019-2024_clean.rds")) %>%
  group_by(business) %>%
  ...
  ungroup() %>%
  # Some quote cleaning for 2021 + 2022 quotes
  mutate(meaningful_action = gsub(": https://corporate.homedepot.com/sites/default/files/THD_2021ESGReport_singlepages_0.pdf",
         ...
         meaningful_action = gsub("\\(https://www.momentive.ai/en/newsroom/momentive-launches-workplace-equity-iq-to-help-companies-measure-dei/\\)",
                                  "",
                                  meaningful_action),
         meaningful_action = gsub(": https://us14.campaign-archive.com/\\?u=4621823774f0db0a17c602b29&id=cbab97a8e6",
                                  "",
                                  meaningful_action),
         meaningful_action= gsub("<br>",
                                 " ",
                                 meaningful_action),
         meaningful_action = gsub(": https://corporate.homedepot.com/sites/default/files/2022-10/2022_ESG_Report_final_4.pdf",
                                  "",
                                  meaningful_action),
         meaningful_action = gsub(" https://djcoregon.com/news/2022/03/22/killian-pacifics-volta-project-meets-dei-goals-with-namc-or-guidance/",
                                  "",
                                  meaningful_action),
         meaningful_action = case_when(meaningful_action %in% 
                                         c("G", 
                                           ...
                                           "Not applicable",
                                           "I have only been at Alchemy for eight months. During these eight months, they have not.",
                                           "n/a",
                                           "NA",
                                           "Not sure") ~ "No statement provided",
...
questions <- readxl::read_excel(here("data/viz-questions_2023.xlsx")) %>%
  select(word_doc_q, survey_item, question_text, present_2023) %>%
  filter(is.na(present_2023)) |> 
